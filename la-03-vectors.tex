...
\begin{frame}
  ...
  \end{figure}
Instead of providing the coordinates $(x,y)$ of a complex number, it
...
\begin{frame}
  ...
  \end{equation}
This formula makes multiplication, division, exponentiation and
finding roots of complex numbers in polar form more simple.
\begin{block}{DeMoivre's Formula}
  $\left(re^{i\theta}\right)^{n}=r^{n}e^{in\theta}$
\end{block}
Note that there are $m$ $m$-th roots of complex numbers.
\end{frame}
