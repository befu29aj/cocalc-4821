...
\begin{frame}
  ...
  \begin{enumerate}
  \item $v_{1},{\ldots},v_{k}$ are linearly independent if and only if
    at least one of these vectors is a linear combination of the
    others
  \item The columns of an augmented matrix $A$ are linearly
    independent if and only if the rank of $A$ is $k$ (the number of
    columns).
...
\begin{frame}
  ...
  is a basis of $V$ if and only if $v_{1},{\ldots},v_{k}$ are linearly
  ...
  \end{block}
  There is a theorem that tells us that if a vector space has two
  bases, then the cardinality of the first base equals the cardinality
  of the second base. Thus, if a finite base exists, it makes sense to
  define the \alert{dimension} of the vector space to be the
  cardinality of that base.
\end{frame}
