...
\item The Fibonacci sequence is $1,1,2,3,5,8,13,21,34,{\ldots}$. It is
  ...
  an explicit (not recursive) formula for the $n$-term?
...
\begin{frame}
  ...
  is an \alert{eigenvalue} if an only if there exists an \alert{eigenvector}
  $X\neq{}0$ such that
  \begin{equation}
    \label{eq:raeshaez}
    AX=\lambda{}X
...
\begin{frame}
  ...
  \begin{block}{Characteristic Polynomial}
    The eigenvalues os a square matrix $A$ are the roots (solutions)
...
S=\{X|X=s_{1}\left[
    \begin{array}{c}
      1\\
      1
    \end{array}\right],s_{1}\in\mathbb{R}\}
  \end{equation}
\end{frame}
